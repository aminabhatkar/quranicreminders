...
--------------------
Sends a preview of the dua carousel slides + caption to the owner via
Telegram, waits for approval, and returns the final decision.

Flow:
    1. Send all 3 carousel slides as a media group
    2. Send the dua details (title, arabic, surah/verse) as a text summary
    3. Send caption + hashtags with inline keyboard: [✅ Post] [❌ Skip] [✏️ Edit Caption]
...
def request_approval(
    ...
    caption:     str,
    hashtags:    str,
    quote:       dict,   # dua dict — kept as 'quote' for backwards compat with main.py
) -> dict:
    """
    Sends the carousel preview to Telegram and waits for owner approval.
    ...
    """
    dua = quote   # rename locally for readability

    print("\nSending Telegram preview for approval...")

    # Preserve the original Claude-generated caption — used on timeout.
    original_cap = caption

    # ── Send all 3 carousel slides ────────────────────────────────────────────
    slide_paths = [
        image_paths["slide_1"],
        image_paths["slide_2"],
        image_paths["slide_3"],
    ...
    preview_caption = (
        f"🕌 *Dua Preview — {dua['title']}*\n\n"
        f"`{dua['arabic']}`\n\n"
        f"_{dua['surah']}, {dua['verse']}_"
    )
    _send_media_group(slide_paths, preview_caption)

    # ── Send caption + hashtags + action buttons ──────────────────────────────
    caption_preview = (
        f"*📝 Caption:*\n{caption}\n\n"
        f"*#️⃣ Hashtags:*\n`{hashtags[:120]}...`\n\n"
